...
**Clinic model (TASK-038).** The ledger belongs to a **clinic**, not a user: every document carries `clinicId` and `useFirestore(clinicId)` filters on it, so two accounts see the same books. A user belongs to exactly one clinic, which is why `memberships/{uid}` is keyed by uid alone — one rule-side document read answers both "is this a member" and "which ledger". `session = { actorId, clinicId, role }` travels with every write: `userId` on a document still means **who did it** (it always did), `clinicId` means which ledger. `ownerFields` omits `clinicId` entirely when it is absent — writing `clinicId: null` would trip the rule's `'clinicId' in request.resource.data` check and deny every write.
...
`update` and `delete` are deliberately **separate** rules. `canUpdateOwned()` requires the old *and* the new owner to be the same user: checking only `resource.data.userId` let a user rewrite their own doc's `userId` and inject records into another clinic's ledger (found by the emulator suite, BAKIM-002). `delete` cannot make that check — `request.resource` does not exist on a delete. Tightening this is safe for `revertPaymentOperations` because `snapshotOf` strips only `id` and `rev`, so `userId` survives the `set(ref, before)` restore.
